...
cdc_resp = tweepy.Cursor(api.search_tweets, cdc_query,
                         lang='en', geocode=geo).items(100)
mask_resp = tweepy.Cursor(api.search_tweets, mask_query,
                          lang='en', geocode=geo).items(100)
vaccine_resp = tweepy.Cursor(
    api.search_tweets, vaccine_query, lang='en', geocode=geo).items(100)
trump_resp = tweepy.Cursor(
    api.search_tweets, trump_query, lang='en', geocode=geo).items(100)
biden_resp = tweepy.Cursor(
    api.search_tweets, biden_query, lang='en', geocode=geo).items(100)


cdc_rows = []
for tweet in cdc_resp:
    cdc_rows.append([re.sub(r'\n|\t', ' ', tweet.text)])
with open(f'data/twitter_raw/{ab}', 'w', encoding='utf-8') as file:
